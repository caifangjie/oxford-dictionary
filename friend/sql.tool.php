...
class Sql
{
	...
	public function __construct()
	{
		...
		public function searchUserInfo($province, $city,$age_list, $sex, $tag)
		{
			...
			//查询省份的中文名字和省级城市名称对应的表
			/* $sql = "select cityname, pro_name from ".$this->province." where pname = '$province' ";
			$result = $this->conn->query($sql);
			if($result->num_rows > 0)
			{
				$temp = $result->fetch_assoc();
				$province_name_chinese = $temp['pro_name'];
				$city_name_tbl = $temp['cityname'];
				$result->free();
			} */
			
			//查询城市的中文名字
			/* $sql = "select chname from ".$city_name_tbl." where enname  = '$city'";
			$result = $this->conn->query($sql);
			if($result->num_rows > 0)
			{
				$temp = $result->fetch_assoc();
				$city_name_chinese = $temp['chname'];
				$result->free();
			} */
			...
			//根据年龄，性别，特征返回用户信息
			$sql = "select info.name, info.sex, info.age, info.qq,info.area, info.tag, info.intro, img.image from 
			(select name, sex, age, qq,area, tag, intro, image_id from $vertical_city_name_tbl 
			where age = '$age' and sex = '$sex' and tag = '$tag' and show_or_hidden='yes') as info, {$this->image_table} as img
			...
			if($result->num_rows > 0)
			{
				...
				{
					$user_info[] = array('name'=>$row[0], 'sex'=>$row[1],'age'=>$row[2], 'qq'=>$row[3],'area'=>$row[4], 
					'tag'=>$row[5],'intro'=>$row[6],'image'=>$row[7]);
				}
				$result->free();
				return $user_info;
			}
