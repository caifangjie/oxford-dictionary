...
{
	if(preg_match('#<div\s+class\s*=\s*["\']basic\s+clearfix["\']>.*?</div>#is', $string, $block))
	{
			if(preg_match_all('#<li>(.*?)</li>#is', $block[0], $match))
			{
				foreach($match[1] as $item)
				{
					...
					{
						$this->basic_trans .= strip_tags($item)."<br />";
					}
				}
			}
			else
			{
				$this->basic_trans = "没有对应的翻译";
			}
	}
	else
	{
		if(preg_match('#<div\s+class="section unfind">.*?</ul>#is', $string, $block))
		{
			if(preg_match_all('#<li>(.*?)</li>#isu', $block[0], $match))
			{
				if(count($match[1]) > 3)
				{
					$items = array_chunk($match[1], 3)[0];
	...
	public function getBasicTrans()
	{
		return $this->basic_trans;
	}
}
//显式的刷新输出缓存
